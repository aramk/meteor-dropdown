...
TemplateClass.rendered = function() {
  ...
  var setUpDropdown = function() {
    ...
    if (data.selectOptions) {
      ...
      items = Collections.getItems(data.items);
    }
    if (allowEmpty) {
      var emptyItem = {};
  ...
  if (!schemaKey) {
    $input.attr('data-schema-key', false);
  }
  // If the element is contained within an AutoForm, wrap it in the usual elements for fields and
  // add a label.
  var isInForm = $dropdown.closest('form');
  if (isInForm.length > 0) {
